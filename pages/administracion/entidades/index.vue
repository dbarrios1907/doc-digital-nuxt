  ...
  <div class="fill-height entidades" style="min-height: 780px">
    ...
    <div v-if="!isListEmpty" class="mt-10 weight-400">
      ...
      <v-select
        ...
        :class="ismobil"
      />
      <span :class="{ 'ml-3': !ismobil }"
        >resultados de un total de <b>{{ countEntities + (countEntities > 1 ? ' entidades' : ' entidad') }}</b></span
      >
    </div>
    <dx-alert
      v-else
      class="mb-9 mt-10 custom-alert font-14 line-height-18 elevation-0 px-md-0 px-4"
      absolute
      bottom
      right
      type="error"
    ...
    <DialogDetail :dialog="entity_detail_dialog" :items="details" headTitle="Entidad">
      <template v-slot:actions>
        ...
        <dx-button
          ...
          @click="entity_detail_dialog = false"
        >
