<template>
  <div>
    <DocumentTray
      ...
      inboxurl="/documentos/bandeja-visar/details/"
    >
      <template v-slot:actions="{ docid }">
        <v-icon dense @click="openDetails(docid)"> mdi-eye </v-icon>
        <visar-icon class="mx-4" aria-controls @click="getid('dialog2', docid)" />
    ...
    <DocumentDetailModalVisar :docid="docid" v-model="dialog2" @onClose="dialog2 = false" @onCancel="dialog2 = false" />
  </div>
</template>
