  ...
  <v-container class="px-0" dx-userform>
    <v-row no-gutters :class="['mt-1 user-form']">
      <div class="col-sm-12">
        ...
        <div v-else class="weight-700 line-height-31 font-25 mb-10">{{ headtitle }}</div>
      </div>
      <v-col cols="12" class="mt-9">
        <div class="weight-400 line-height-30 font-16">{{ headtext }}</div>
      </v-col>
      <!-- <div class="mt-4" ref="form" > -->
      <v-form ref="form" v-model="valid" class="mt-4" style="min-height: 680px" lazy-validation>
        <div v-if="userid" class="col-md-5 col-sm-12 px-0 mr-10" :class="[ismobil, 'col-container']">
          <v-row no-gutters :class="[{ 'align-center': ismobil }]">
            ...
            <v-col style="max-height: 72px; line-height: 74px">
              Activo
              <v-switch v-model="isBloqueado" class="d-inline-block mt-0 pt-0 success-switch ml-4" style="width: 40px" inset :ripple="false" dense />
            </v-col>
          </v-row>
        </div>
        <div v-if="!ismobil" class="col-md-12 col-sm-12 px-0 mr-10 d-none d-md-flex d-lg-flex d-xl-flex" style="min-height: 30px" />
        ...
        <div v-if="!ismobil" class="col-md-5 col-sm-12 px-0 mr-10 d-none d-md-flex d-lg-flex d-xl-flex" style="min-height: 74px" />
        <div class="col-md-5 col-sm-12 px-0 mr-10" :class="[ismobil, 'col-container']">
          <v-row no-gutters :class="['align-center', ismobil]">
            <v-col cols="auto" class="label-width" :class="['flex weight-400 line-height-30 font-16 py-1', { 'mt-minus-28': !ismobil }]">
              Nombre(s)*
            </v-col>
            <v-col class="mh-72">
              <v-text-field
          ...
          <v-row no-gutters :class="['align-center', ismobil]">
            ...
            <v-col class="mh-72">
              <v-select
                ...
                v-bind="$props"
                required
                :rules="!disabled ? rules.entidadrequired : []"
                :disabled="disabled"
                item-text="name"
                item-value="id"
        ...
        <div class="col-md-5 col-sm-12 px-0 mr-10" :class="[ismobil, 'col-container']">
          <v-row no-gutters :class="['align-center', ismobil]">
            ...
            <div v-if="ismobil" class="col-md-5 col-sm-12 px-0 mr-10" style="min-height: 30px" />
            <v-col>
              <dx-select
                :items="roles_select"
        ...
        <div v-if="!ismobil" class="col-md-12 col-sm-12 px-0 py-0 mr-10 d-none d-md-flex d-lg-flex d-xl-flex" style="min-height: 1px" />
        <div class="col-md-5 col-sm-12 px-0 mr-10" :class="[ismobil, 'col-container']">
          <v-row no-gutters :class="['align-center', ismobil]">
            <v-col cols="auto" class="label-width" :class="['flex weight-400 line-height-30 font-16 py-1', { 'mt-minus-28': !ismobil }]">
              Subrogante
        ...
        <div class="col-md-5 col-sm-12 px-0 mr-10" :class="[ismobil, { 'mb-10': ismobil }, 'col-container']">
          <v-row no-gutters :class="[{ 'align-center': ismobil }]">
            <v-col
              cols="auto"
              :class="['flex weight-400 line-height-30 font-16', { 'mt-3': !ismobil }, { 'mt-minus-28': ismobil }]"
              sryle="height: 100%"
            >
              Activar subrogancia
            </v-col>
...
import { verificationDigit, verifyRut } from '~/shared/utils/rut-validator'
import { lowerCase, trim, escapeRegExp } from '~/shared/utils/string'
export default {
  name: 'Userform',
  props: {
    userid: {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        rutVerifier: [v => !!v || !!this.dv || 'Campo requerido', v => verifyRut(v + '-' + this.dv, false) || 'Rut inválido'],
        dvVerifier: [v => !!v || !!this.dv || ''],
      },
      roles_select: [
        // {
        //   key : 'ROLE_USUARIO',
  ...
  computed: {
    ...
    subrogantesByEntity() {
      const getByEntity = this.$store.getters['usuarios/getByEntity']
      const usuarios = getByEntity(this.entidad)
  ...
  mounted() {
    ...
    this.seguidores = activos.map(({ id, nombres }) => {
      return {
        id,
        name: nombres,
      }
  ...
  methods: {
    ...
    async submit() {
      ...
      const index = entlist.findIndex(obj => obj.id == this.entidad)
      ...
      if (this.$refs.form.validate()) {
        ...
        try {
          if (this.userid) resp = await this.$store.dispatch('usuarios/updateUser', user)
          else resp = await this.$store.dispatch('usuarios/insertUser', user)
          // eslint-disable-next-line no-empty
        } catch (error) {}
