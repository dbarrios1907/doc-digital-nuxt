    ...
    <div class="b-upload font-large">
      <dx-button color="primary" outlined class="btn-upload" @click="dialog = true">
        <div class="btn-upload-content">
          ...
          <span class="text-underline">{{ buttonText }}</span>
        </div>
      </dx-button>
    </div>
    <v-dialog
        ...
        <validation-observer ref="observer" v-slot="{ invalid }">
          <form>
              ...
              <v-row class="mt-5">
                ...
                <v-col cols="12" sm="12" md="8" lg="8" class="py-sm-0 px-xs-0">
                  <validation-provider v-slot="{ errors }" name="url" rules="required|url">
                    <dx-text-field v-model="url" flat solo outlined placeholder="Escriba la URL" required :error-messages="errors" />
                  </validation-provider>
                </v-col>
              </v-row>
              <v-row>
                ...
                <v-col cols="12" sm="12" md="8" lg="8" class="py-sm-0 px-xs-0">
                  <dx-text-field v-model="descripcion" flat solo outlined placeholder="Escriba la descripción" />
                </v-col>
              </v-row>
            </v-card-text>
            <v-card-actions class="px-lg-10 px-md-10 pb-10 mt-2 d-flex justify-center">
              ...
              <dx-button color="primary" :disabled="invalid" class="text-none ml-5" @click="save">
